...
tags: [Cloud]
META DESCRIPTION: Cloud hunting isn't on-prem hunting with different logs. Here's what genuinely changes and where the fundamentals still hold.
---
...
The platform-specific skills CloudTrail analysis, Azure Sentinel queries, GuardDuty tuning are worth their own deep dives, and we cover AWS and Azure specifically in the next two posts in this series. But get the fundamentals here solid first, because platform-specific tooling knowledge without the underlying hunting discipline just produces someone who's very good at running queries and not very good at knowing which queries to run. ThreatHuntLabs' cloud hunting fundamentals module builds that foundation before diving into any single platform the right order to learn it in, even if it's tempting to skip straight to the tool-specific content.
