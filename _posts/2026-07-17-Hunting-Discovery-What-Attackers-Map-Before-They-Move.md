...
tags: [Discovery]
description: Detecting the reconnaissance and enumeration techniques attackers use after initial access, before deciding where to move next.
---



![Discovery actions mapped as the decisions an attacker makes before moving](/assets/img/threat-hunting/attacker-path.svg)
...
A confirmed discovery finding is genuinely valuable beyond itself, because what an attacker chooses to enumerate tells you something about what they're likely to target next. In one plausible case, a hunt confirms an attacker enumerated file shares specifically related to a finance department, rather than broadly across the entire environment—that narrows your subsequent hunting priorities meaningfully, suggesting the next stages of the intrusion (lateral movement, collection) are more likely to focus on that specific area than elsewhere, which is worth acting on immediately rather than waiting for those later stages to confirm the same thing independently.
