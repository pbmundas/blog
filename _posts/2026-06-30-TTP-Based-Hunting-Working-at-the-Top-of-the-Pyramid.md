...
tags: [TTP, Pyramid of Pain, threat hunting]
description: Why hunting attacker tactics, techniques, and procedures instead of artefacts produces the most durable, hardest-to-evade detections.
image:
  path: /assets/img/threat-hunting/pyramid-of-pain.svg
  alt: "Pyramid of Pain progressing from hashes to attacker tactics, techniques, and procedures"
---

## What you will learn

- Explain why behavioral detections usually outlast atomic indicators.
- Move from a broad technique to a locally specific procedure.
- Convert a validated TTP hunt into maintainable detection coverage.
...
## Write a procedure-level hypothesis

Select one ATT&CK technique, then specify how it would be performed in your environment: likely tool or native capability, parent process, identity, target, sequence, and evidence. List normal administrative procedures that overlap. This moves the hunt from “search for a technique” to behavior that can actually be tested.

## Key takeaway

TTP hunting is durable because changing behavior costs an attacker more than changing an indicator. That durability depends on local procedure-level detail, not a broad technique label.
