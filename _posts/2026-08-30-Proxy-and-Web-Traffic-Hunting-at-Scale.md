...
tags: [Proxy]
META DESCRIPTION: Millions of web requests a day hide a handful of real threats. Here's how to hunt proxy logs using categorization and anomaly detection.
---
...
Proxy logs are one of the richest, most underused hunting data sources most organizations already have sitting in a SIEM somewhere, mostly reviewed only after something else already triggered an investigation. Flipping that using proxy analysis proactively rather than reactively catches things nothing else in your stack would surface first. ThreatHuntLabs' proxy hunting module walks through building this categorization-plus-anomaly scoring pipeline against real captured web traffic, including planted malicious activity buried in genuinely boring background noise which is exactly the condition you'll be hunting under for real.
